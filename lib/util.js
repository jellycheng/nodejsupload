...
_.touch = function(path, mtime){
    ...
    if(mtime instanceof Date){
        ...
        cjs.log.error('invalid argument [mtime]');
    }
...
function getFileTypeReg(type){
    var map = [], ext = cjs.config.get('project.fileType.' + type);
    if(type === 'text'){
        map = TEXT_FILE_EXTS;
    } else if(type === 'image'){
        map = IMAGE_FILE_EXTS;
    } else {
        cjs.log.error('invalid file type [' + type + ']');
    }
    if(ext && ext.length){
...
_.md5 = function(data, len){
    ...
    len = len || cjs.config.get('project.md5Length', 7);
...
_.read = function(path, convert){
    ...
    if(_exists(path)){
        ...
        cjs.log.error('unable to read file[' + path + ']: No such file or directory.');
    }
    return content;
};
    ...
    function normalize(pattern){
        ...
        switch (type){
            ...
            case '[object RegExp]':
                return pattern;
            default:
                cjs.log.error('invalid regexp [' + pattern + '].');
        }
    }
...
_.find = function(rPath, include, exclude, root){
    ...
    if(path){
        ...
        cjs.log.error('unable to find [' + rPath + ']: No such file or directory.');
    }
    return list.sort();
};
...
_.del = function(rPath, include, exclude){
    ...
    if(rPath && _.exists(rPath)) {
        ...
        if(/^(?:\w:)?\/$/.test(path)){
            cjs.log.error('unable to delete directory [' + rPath + '].');
        }
        ...
        //cjs.log.error('unable to delete [' + rPath + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.copy = function(rSource, target, include, exclude, uncover, move){
    ...
    if(source){
        ...
        cjs.log.error('unable to copy [' + rSource + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.pipe = function(type, callback, def){
    var processors = cjs.config.get('modules.' + type, def);
    if(processors){
    ...
    var writer = fs.createWriteStream(tmp),
        http_err_handler = function(err){
            ...
            if(callback){
                ...
                cjs.log.error('request error [' + msg + ']');
            }
        },
                            ...
                            fs
                                ...
                                .on('error', function(err){
                                    if(callback){
                                        ...
                                        cjs.log.error('extract tar file [' + tmp + '] fail, error [' + err + ']');
                                    }
                                })
                                .on('end', function(){
...
_.upload = function(url, opt, data, content, subpath, callback){
    if(typeof content === 'string'){
        ...
        cjs.log.error('unable to upload content [' + (typeof content) + ']');
    }
    data = data || {};
    var endl = '\r\n';
    ...
    _.download(url, function(err){
        if(err){
            if(opt.error){
                ...
                cjs.log.error( 'unable to download component [' +
...
_.readJSON = function(path){
    ...
    try {
        ...
        cjs.log.error('parse json file[' + path + '] fail, error [' + e.message + ']');
    }
    return result;
};
...
_.nohup = function(cmd, options, callback){
    ...
    if(IS_WIN){
        ...
        return exec('cscript.exe /nologo "' + file + '"', options, function(error, stdout){
            if(stdout != '0'){
                cjs.log.error('exec command[' + cmd + '] fail.');
            }
            fs.unlinkSync(file);
        ...
        return exec('nohup ' + cmd + ' > /dev/null 2>&1 &', options, function(error, stdout){
            if(error !== null) {
                cjs.log.error('exec command[' + cmd + '] fail, stdout [' + stdout + '].');
            }
            if(typeof callback === 'function') {
                callback();
            }
